...
if(isset($_POST['submit']))
{
	...
	if($password != $repassword) {$error[]=' Parola reintrodusa gresit ! ';}
	...
	{    // verifica unicitatea username-ului si a parolei introduse //
		...
		if(mysql_num_rows($rez_unicitate)==0) 	//daca interogarea asupra bazei de date returneza 0 randuri //
		{
			$rez_insertie = mysql_query("INSERT INTO user (id_user,username,password) VALUES ('','$username','$password') ") or die(mysql_error());
			if($rez_insertie) { header('Location:login.php');}
			else { $error[] =' Reintroduceti datele de inregistrare ! '; }
		}
		else { $error[] =' Usernameul introdus nu este disponibil . ';}
	}
